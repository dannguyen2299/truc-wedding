function renderEnvelopeCatalog(rows) {
  ...
  const items = envelopeRows(rows);
  catalog.innerHTML = items.map((item) => {
    const name = escapeHtml(item.name || 'Phong bì');
...
async function loadEnvelopeCatalog() {
  ...
  const catalog = document.getElementById('envelopeCatalog');
  status.textContent = 'Đang tải danh sách phong bì…';
  catalog.setAttribute('aria-busy', 'true');
  retry.hidden = true;
  try {
    const response = await fetch(valuesUrl(PRICING_CONFIG.SHEET_ID,
      `${PRICING_CONFIG.ENVELOPES_TAB_NAME}!A:D`, `?key=${encodeURIComponent(PRICING_CONFIG.API_KEY)}`));
    const data = await parseResponse(response, 'Không tải được phong bì');
    const count = renderEnvelopeCatalog(rowsToObjects(data.values));
    status.textContent = count ? '' : 'Chưa có phong bì nào.';
  } catch (error) {
    catalog.replaceChildren();
    status.textContent = 'Không tải được danh sách phong bì. Vui lòng thử lại.';
    retry.hidden = false;
